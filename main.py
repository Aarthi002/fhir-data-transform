...
from pyspark.sql.functions import col, explode, when,size,expr,count,lit
import os
...
json_file_path = "data/Abbey813_Price929_83524678-9bff-93b7-ef89-d7f5390072ff.json"
...
def perform_reconciliation(input_df, resource_type):
    ...
    discrepancies = recon_result.filter(col("status") != "Match") \
                                .select(col("input_key").alias("patient_id"), col("status"), col("discrepancy_type"))
